...
test('analytics scopes every aggregation by branch/date and groups quantities by product/unit', async t => {
  ...
  const chain = data => { const query = { select: () => query, sort: () => query, populate: () => query, limit: value => { assert.equal(value,10); return query; }, lean: async () => data }; return query; };
  ...
  assert.deepEqual(result.data.summary,{total:3,in_transit:1,delivered:2,cancelled:0,wastage:4});
